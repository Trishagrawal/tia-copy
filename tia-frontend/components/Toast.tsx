...
function ToastContainer({ toasts, onRemove }: ToastContainerProps) {
  ...
  const getStyles = (type: ToastType) => {
    const baseStyles = "rounded-lg shadow-lg p-4 flex items-start gap-3 text-sm";
    switch (type) {
      case "success":
  ...
  return (
    <div className="fixed bottom-0 right-0 p-4 space-y-2 z-50 max-w-md pointer-events-none">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`${getStyles(toast.type)} pointer-events-auto animate-fade-in-up`}
        >
